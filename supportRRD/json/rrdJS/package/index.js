//why is this also index.js, why is it held inside package, what does this mean, my brain oof
"use strict";
...
var RRDStream = (module.exports = function (options) {
  if (!(this instanceof RRDStream)) return new RRDStream(options);
...
RRDStream.prototype._transform = function (chunk, encoding, done) {
  ...
  if (
    this.header &&
    !this.header.ds &&
    this.buffer.length >= this.header.header_size
  ) {
    ...
    this.header.rra.forEach(
      function (rra, index) {
        ...
        rra_start =
          this.options.start > rra_start ? this.options.start : rra_start;
  ...
  if (!this.header || !this.header.ds) return done();
...
RRDHeader.prototype.parse = function (data) {
  ...
  for (var i = 0; i < this.ds_cnt; i++, offset += 120) {
    ...
    var type = data
      .toString("ascii", offset + 20, offset + 40)
      .split("\x00")[0];
  ...
  for (i = 0; i < this.ds_cnt; i++, offset += 112) {
    this.ds[i].last_ds = data
      .toString("ascii", offset, offset + 30)
      .split("\x00")[0];
...
RRDRRA.prototype.push = function (row) {
  ...
  if (this.row_count > this.cur_row) {
    ...
    for (var i = 0; i < this.cdp_prep.length; i++)
      dict[this.ds[i].name]["data"][this.step].push({
        x_value: this.timestamp,
        y_value: row.readDoubleLE(i * 8),
      });
